...
Program TackyGenerator::generate()
{
    Program program = transform_program(*dynamic_cast<parser::Program*>(m_ast.get()));
    transform_symbols_to_tacky(program);
    return program;
}

void TackyGenerator::transform_symbols_to_tacky(Program& tacky_program)
{
    auto& top_levels = tacky_program.definitions;
    ...
    for (const auto& p : m_symbol_table->symbols()) {
        ...
        if (std::holds_alternative<StaticAttribute>(entry.attribute)) {
            ...
            if (std::holds_alternative<InitialValue>(static_attr.init)) {
                ...
                top_levels.emplace_back(StaticVariable(variable_name, global, initial_value));
            } else if (std::holds_alternative<TentativeInit>(static_attr.init)) {
                top_levels.emplace_back(StaticVariable(variable_name, global, 0));
...
Value TackyGenerator::transform_expression(parser::Expression& expression, std::vector<Instruction>& instructions)
{
    if (parser::ConstantExpression* constant_expression = dynamic_cast<parser::ConstantExpression*>(&expression)) {
        return Constant(constant_expression->value);
    } else if (parser::UnaryExpression* unary_expression = dynamic_cast<parser::UnaryExpression*>(&expression)) {
        Value src = transform_expression(*unary_expression->expression, instructions);
        std::string dst_name = m_name_generator->make_temporary();
        TemporaryVariable dst = TemporaryVariable(dst_name);
        Value dst_copy = TemporaryVariable(dst);
        UnaryOperator op = transform_unary_operator(unary_expression->unary_operator);
        instructions.emplace_back(UnaryInstruction(op, std::move(src), std::move(dst)));
        return dst_copy;
        ...
        if (binary_expression->binary_operator == parser::BinaryOperator::AND) {
            Value src1 = transform_expression(*binary_expression->left_expression, instructions);
            std::string false_label = m_name_generator->make_label("and_false");
            instructions.emplace_back(JumpIfZeroInstruction(std::move(src1), false_label));
            Value src2 = transform_expression(*binary_expression->right_expression, instructions);
            instructions.emplace_back(JumpIfZeroInstruction(std::move(src2), false_label));
            std::string result = m_name_generator->make_temporary();
            {
                TemporaryVariable result_var = TemporaryVariable(result);
                instructions.emplace_back(CopyInstruction(1, std::move(result_var)));
            }

            std::string end_label = m_name_generator->make_label("and_end");
            instructions.emplace_back(JumpInstruction(end_label));
            instructions.emplace_back(LabelInstruction(false_label));
            {
                TemporaryVariable result_var = TemporaryVariable(result);
                instructions.emplace_back(CopyInstruction(0, std::move(result_var)));
            }
            instructions.emplace_back(LabelInstruction(end_label));
            TemporaryVariable result_var = TemporaryVariable(result);
            return result_var;
            ...
            instructions.emplace_back(JumpIfNotZeroInstruction(std::move(src1), true_label));
            Value src2 = transform_expression(*binary_expression->right_expression, instructions);
            instructions.emplace_back(JumpIfNotZeroInstruction(std::move(src2), true_label));
            std::string result = m_name_generator->make_temporary();
            {
                TemporaryVariable result_var = TemporaryVariable(result);
                instructions.emplace_back(CopyInstruction(0, std::move(result_var)));
            ...
            instructions.emplace_back(JumpInstruction(end_label));
            instructions.emplace_back(LabelInstruction(true_label));
            {
                TemporaryVariable result_var = TemporaryVariable(result);
                instructions.emplace_back(CopyInstruction(1, std::move(result_var)));
            }
            ...
            instructions.emplace_back(BinaryInstruction(op, std::move(src1), std::move(src2), std::move(dst)));
            return dst_copy;
        }
    } else if (parser::VariableExpression* variable_expression = dynamic_cast<parser::VariableExpression*>(&expression)) {
        return TemporaryVariable(variable_expression->identifier.name);
        ...
        Value right = transform_expression(*assignment_expression->right_expression, instructions);
        instructions.emplace_back(CopyInstruction(std::move(right), TemporaryVariable(variable->identifier.name)));
        return TemporaryVariable(variable->identifier.name);
        ...
        // Condition
        Value cond = transform_expression(*conditional_expression->condition, instructions);
        instructions.emplace_back(JumpIfZeroInstruction(std::move(cond), false_label));

        // True Instructions
        Value true_value = transform_expression(*conditional_expression->true_expression, instructions);
        {
            // result = true_expression
            TemporaryVariable result_var = TemporaryVariable(result);
            instructions.emplace_back(CopyInstruction(std::move(true_value), std::move(result_var)));
        }
        instructions.emplace_back(JumpInstruction(end_label));

        // False Instructions
        instructions.emplace_back(LabelInstruction(false_label));
        Value false_value = transform_expression(*conditional_expression->false_expression, instructions);
        {
            // result = false_expression
            TemporaryVariable result_var = TemporaryVariable(result);
            instructions.emplace_back(CopyInstruction(std::move(false_value), std::move(result_var)));
        }

        instructions.emplace_back(LabelInstruction(end_label));
        return TemporaryVariable(result);
        ...
        std::string result = m_name_generator->make_temporary();
        TemporaryVariable result_var = TemporaryVariable(result);
        instructions.emplace_back(FunctionCallInstruction(function_call_expression->name.name, std::move(args), std::move(result_var)));
        return TemporaryVariable(result);
    } else {
...
void TackyGenerator::transform_statement(parser::Statement& statement, std::vector<Instruction>& instructions)
{
    if (parser::ReturnStatement* return_statement = dynamic_cast<parser::ReturnStatement*>(&statement)) {
        Value value = transform_expression(*(return_statement->expression.get()), instructions);
        instructions.emplace_back(ReturnInstruction(std::move(value)));
    } else if (parser::ExpressionStatement* expression_statement = dynamic_cast<parser::ExpressionStatement*>(&statement)) {
        Value value = transform_expression(*(expression_statement->expression.get()), instructions);
        // value is discarded
    } else if (parser::IfStatement* if_statement = dynamic_cast<parser::IfStatement*>(&statement)) {
        Value cond = transform_expression(*(if_statement->condition.get()), instructions);
        if (!if_statement->else_statement.has_value()) {
            std::string end_label = m_name_generator->make_label("if_end");
            instructions.emplace_back(JumpIfZeroInstruction(std::move(cond), end_label));
            transform_statement(*if_statement->then_statement, instructions);
            instructions.emplace_back(LabelInstruction(end_label));
        } else {
            std::string else_label = m_name_generator->make_label("else");
            std::string end_label = m_name_generator->make_label("if_end");
            instructions.emplace_back(JumpIfZeroInstruction(std::move(cond), else_label));
            transform_statement(*if_statement->then_statement, instructions);
            instructions.emplace_back(JumpInstruction(end_label));
            instructions.emplace_back(LabelInstruction(else_label));
        ...
        instructions.emplace_back(JumpInstruction(break_label));
        ...
        instructions.emplace_back(JumpInstruction(continue_label));
        ...
        instructions.emplace_back(LabelInstruction(start_label));
        transform_statement(*do_while_statement->body, instructions);
        instructions.emplace_back(LabelInstruction(continue_label));
        Value cond = transform_expression(*(do_while_statement->condition.get()), instructions);
        instructions.emplace_back(JumpIfNotZeroInstruction(std::move(cond), start_label));
        instructions.emplace_back(LabelInstruction(break_label));
        ...
        instructions.emplace_back(LabelInstruction(continue_label));
        Value cond = transform_expression(*(while_statement->condition.get()), instructions);
        instructions.emplace_back(JumpIfZeroInstruction(std::move(cond), break_label));
        ...
        if (for_statement->condition.has_value()) {
            Value cond = transform_expression(*(for_statement->condition.value().get()), instructions);
            instructions.emplace_back(JumpIfZeroInstruction(std::move(cond), break_label));
        }
        ...
        instructions.emplace_back(JumpInstruction(start_label));
        instructions.emplace_back(LabelInstruction(break_label));
...
void TackyGenerator::transform_declaration(parser::Declaration& declaration, std::vector<Instruction>& instructions)
{
    if (parser::VariableDeclaration* variable_declaration = dynamic_cast<parser::VariableDeclaration*>(&declaration)) {
        // We do not generate any code for local variable declartions with static or external specifiers
        if (variable_declaration->storage_class == parser::StorageClass::NONE && variable_declaration->expression.has_value()) {
            Value value = transform_expression(*variable_declaration->expression.value(), instructions);
            instructions.emplace_back(CopyInstruction(std::move(value), TemporaryVariable(variable_declaration->identifier.name)));
        }
...
void TackyGenerator::transform_for_init(parser::ForInit& for_init, std::vector<Instruction>& instructions)
{
...
void TackyGenerator::transform_block_item(parser::BlockItem& block_item, std::vector<Instruction>& instructions)
{
...
void TackyGenerator::transform_block(parser::Block& block, std::vector<Instruction>& instructions)
{
...
std::optional<FunctionDefinition> TackyGenerator::transform_function(parser::FunctionDeclaration& function)
{
    if (function.body.has_value()) {
        ...
        body.emplace_back(ReturnInstruction(Constant(0)));
        bool global = std::get<FunctionAttribute>(m_symbol_table->symbol_at(function.name.name).attribute).global;
        return FunctionDefinition(function.name.name, global, params, std::move(body));
    }

    return std::nullopt;
}

std::optional<TopLevel> TackyGenerator::transform_top_level_declaraiton(parser::Declaration& declaration)
{
...
Program TackyGenerator::transform_program(parser::Program& program)
{
    std::vector<TopLevel> definitions;
    for (auto& parser_decl : program.declarations) {
        auto def = transform_top_level_declaraiton(*parser_decl);
        if (def.has_value()) {
            definitions.emplace_back(std::move(def.value()));
        }
    }
